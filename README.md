...
- `repodex search --q "<query>" [--top_k N] [--score] [--no-format] [--json]` ??" run ranked keyword search (caps: top_k max 20).
...
Use `--no-format` to remove indentation and escape snippet lines starting with `>`, `-`, or `@` by prefixing `\`. Use `--score` to include `~<score>` on the hit line. Use `--json` for the raw JSON array.
